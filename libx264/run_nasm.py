...
"""A wrapper to run nasm.

Its main job is to provide a Python wrapper for GN integration, and to write
the makefile-style output nasm generates in stdout to a .d file for dependency
management of .inc files.

Run with:
  python run_nasm.py <nasm_binary_path> <all other nasm args>

Note that <all other nasm args> must include an explicit output file (-o). This
script will append a ".d" to this and write the dependencies there. This script
will add "-M" to cause nasm to write the deps to stdout, so you don't need to
specify that.
"""
...
# Extract the output file name from the nasm command line so we can generate a
...
# Now generate the .d file listing the dependencies. The -M option makes nasm
...
# However, outputting deps seems faster than actually assembling, and nasm is
